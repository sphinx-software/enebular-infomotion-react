...
/* Prototype Methods */
  ...
  }, {'./core.js': 3, './done.js': 4, './es6-extensions.js': 5, './finally.js': 6, './node-extensions.js': 8, './synchronous.js': 9}],
...
// https://github.com/tildeio/rsvp.js/blob/cddf7232546a9cf858524b75cde6f9edf72620a7/lib/rsvp/asap.js
  ...
  12: [function (require, module, exports) {
    ...
    window.EnebularIntelligence = {
      Plugins: {},
      SchemaProcessor: function (schema, defaultSettings) {
        return {
          schema: schema,
      ...
      register: function (pluginName, Plugin) {
        var pluginSetId = document.currentScript.dataset['id']
        var pluginSetTitle = document.currentScript.dataset['title']
        var pluginId = pluginName
        var pluginTitle = pluginName
        if (pluginSetId) {
          pluginId = pluginSetId + '-' + pluginName
        }
        Plugin._meta = {
          groupId: pluginSetId,
          groupTitle: pluginSetTitle,
          id: pluginId,
          displayName: pluginTitle
        }
        window.EnebularIntelligence.Plugins[pluginId] = Plugin
      }
    }
 ...
 */
    ...
    window.addEventListener('load', function () {
      window.parent.postMessage({
        iframe_id: location.hash.substr(1),
        event: 'loaded_graph'}, '*')
    })

    window.addEventListener('message', function (e) {
      if (e.data.method == 'loadScript' || e.data.method == 'loadResource') {
        exec_message(e.data)
      } else {
        recv_queue.push(e.data)
        process_message()
      }
    })
    ...
    function exec_message (data) {
    console.log("exec_message", data);
      if (data.method == 'loadScript') {
        var pluginPath = data.args.pluginPath
        var pluginNames = data.args.pluginNames
        loadByPluginName(pluginPath, pluginNames).then(function () {
          _is_loaded = true
          fireSettingsEvent()
        }).catch(function (err) {
          console.error(err.stack)
        })
      } else if (data.method == 'loadResource') {
        var resources = data.args.resources
      ...
      } else if (data.method == 'createGraph') {
        
        console.log("createGraph",data)

        var id = data.args.id
        var iframe_id = data.args.iframe_id
        var type = data.args.type
        var settings = data.args.settings
        var options = data.args.options
        var GraphInner = EnebularIntelligence.Plugins[type]
        var inner = new GraphInner(settings, options)
        var rootDom = document.getElementById('root')

        if (rootDom.childNodes.length > 0) { rootDom.removeChild(rootDom.firstChild) }
        rootDom.appendChild(inner.getEl())

        inner.onDrillDown(function (filter) {
          window.parent.postMessage({
            iframe_id: location.hash.substr(1),
            event: 'ondrilldown',
            body: {
        ...
        inner.setLocalStorageData = function (value) {
          window.parent.postMessage({
            iframe_id: location.hash.substr(1),
            event: 'setlocalstoragedata',
            body: {
              graph_id: id,
              value: value
            }
        ...
        inner.updateSettings = function (settings, options) {
          window.parent.postMessage({
            iframe_id: location.hash.substr(1),
        ...
        graphInnerTable[iframe_id] = inner

      } else if (data.method == 'showGraph') {
        var id = data.args.id
        var rootDom = document.getElementById('root')
        if (rootDom.childNodes.length > 0) { rootDom.removeChild(rootDom.firstChild) }
        rootDom.appendChild(graphInnerTable[id].getEl())
      } else if (data.method == 'addData') {
        var id = data.args.id
        var _data = data.args.data
        invoke_method(id, 'addData', _data)
      } else if (data.method == 'clearData') {
        var id = data.args.id
        invoke_method(id, 'clearData')
      } else if (data.method == 'resize') {
        var id = data.args.id
        var options = data.args.options
        invoke_method(id, 'resize', options)
      } else if (data.method == 'zoom') {
        var id = data.args.id
        var point = data.args.point
        invoke_method(id, 'zoom', point)
      }
      function invoke_method (id, method, arg1) {
        if (id) {
          if(graphInnerTable[id][method]){ 
            graphInnerTable[id][method](arg1)
          }
        } else {
          Object.keys(graphInnerTable).forEach(function (id) {
            var inner = graphInnerTable[id]
            if (inner[method]) inner[method](arg1)
          })
        }
      }
    }

    function fireSettingsEvent () {
      var graphNames = Object.keys(EnebularIntelligence.Plugins)
      var graphPluginSettgins = {}
      graphNames.forEach(function (graphName) {
        var p = EnebularIntelligence.Plugins[graphName]
        graphPluginSettgins[graphName] = {
          id: p._meta.id,
          displayName: p._meta.displayName,
          groupId: p._meta.groupId,
          groupTitle: p._meta.groupTitle,
          settings: p.settings
        }
      })
      window.parent.postMessage({
        ...
        event: 'settings',
        body: {
          graphPluginSettgins: graphPluginSettgins
        }
      }, '*')
    }

    function loadManyByPluginName (pluginPath, pluginNames) {
      return Promise.all(pluginNames.map(function (pluginName) {
        return loadByPluginName(pluginPath, pluginName)
      }))
    }

    function loadByPluginName (pluginPath, pluginInfo) {
      var that = this
      if (pluginInfo instanceof Array) {
        return loadManyByPluginName(pluginPath, pluginInfo)
      } else {
        if (typeof pluginInfo === 'string') {
          var pluginName = pluginInfo
          var pluginTitle = pluginName
          var jsURL = pluginPath + '/' + pluginName + '/plugin.js'
          var cssURL = pluginPath + '/' + pluginName + '/plugin.css'
        } else {
          var pluginTitle = pluginInfo.title
          var pluginName = pluginInfo.name
          var jsURL = pluginPath + '/' + pluginInfo.jsPath
          var cssURL = pluginPath + '/' + pluginInfo.cssPath
        }
        return new Promise(function (resolve, reject) {
          loadScript(jsURL, pluginName, pluginTitle, function () {
            loadStyleSheet(cssURL, function () {
              resolve(pluginName)
            })
          })
        })
    ...
    function loadResources (resources) {
      ...
      function loadResource (resource) {
        return new Promise(function (resolve, reject) {
          ...
          script.innerHTML = resource.script
          if (resource.name) { script.dataset['id'] = resource.name }
          if (resource.title) { script.dataset['title'] = resource.title }
          style.innerHTML = resource.style
    ...
    function loadScript (src, id, title, callback) {
      var done = false
      var head = window.document.getElementsByTagName('head')[0]
      var script = document.createElement('script')
      if (id) { script.dataset['id'] = id }
